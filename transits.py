...
def find_observable_transits(population, observer,obs_start_time, *args, **kwargs): # constraints=None, obs_start_time = datetime.now(), n_transits=1):
    ...
    for i in range(len(population)):
        if len(population) > 1:
            print("Population length cannot exceed 1. If transits for more than 1 object are desired, use function 'find_all_observable_transits'")
            break
            ...
            print("Object transit midpoint not found. Please use a valid transit midpoint.")
            break
        elif np.isnan(duration) == True:
            print("Object transit duration not found. Please use a valid transit duration.")
            break
        else:
            pass
        ...
        transit_table = QTable(names= ('name','ra','dec','period','transit_midpoint','transit_duration','ingress_time','midpoint_time','egress_time')
                   ,dtype=(population.name.dtype, population.ra.dtype, population.dec.dtype, population.period.dtype
                           ,population.transit_midpoint.dtype, population.transit_duration.dtype,'U','U','U'))
        

        es = EclipsingSystem(primary_eclipse_time=midtransit_time, orbital_period=period, duration=transit_duration)
    
            
        transit_times = es.next_primary_eclipse_time(obs_start_time, n_eclipses=n_transits) #creating the array of mid-transit times
        ing_eg_times = es.next_primary_ingress_egress_time(obs_start_time, n_eclipses=n_transits) #array of ingress/egress times
            

        in_iso = []
        eg_iso = []
        for j in range(len(transit_times)):   
            in_iso= np.append(in_iso, Time(ing_eg_times[j,0], format='iso', scale='utc'))
            eg_iso = np.append(eg_iso, Time(ing_eg_times[j,1], format='iso', scale='utc'))
               
                
        in_eg_arr = np.vstack((in_iso, eg_iso)).T
        obs_arr = is_event_observable(constraints=constraints, observer=observer, target=target, times=transit_times, times_ingress_egress=in_eg_arr) 
        ...
        for i in range(len(ingress)):
            transit_table.add_row(vals=(population.name,ra,dec,period,midpoint,duration, ingress[i].value,midpoint_times[i].value,egress[i].value))
...
def find_all_observable_transits(population,observer,obs_start_time,*args, **kwargs):
    ...
    for i in range(len(population)):
        row = find_observable_transits(population=population[i], observer=observer, constraints=constraints,obs_start_time=obs_start_time
...
def make_airmass_plot(transit_table,observer,savefig=False):
    for i in range(len(transit_table)):
        ...
        plot_airmass(targets=target, observer=observer, time=Time(midpoint_times), brightness_shading=True, altitude_yaxis=True,use_local_tz=True)
        plt.axvline(Time(midpoint_times).plot_date, color='orange', linestyle='--', label = 'mid-transit')
        plt.axvline(Time(ingress).plot_date, color='green', linestyle='--', label = 'ingress')
        plt.axvline(Time(egress).plot_date, color='green', linestyle='--', label = 'egress')
