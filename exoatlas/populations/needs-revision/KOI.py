...
def downloadLatest():
    print(
        "downloading the latest list of confirmed exoplanets from the Exoplanet Archive"
    )
...
class UnconfirmedKepler(TransitingExoplanetsSubset):
    def __init__(self):
        TransitingExoplanetsSubset.__init__(
            self, label="Kepler (candidates)", color="gray", zorder=-1e6
        )
        self.respond_to_color = True
