...
class Observatory(Talker):
    ...
    def __init__(self, abbreviation=None, plan=None):
        ...
        self.location = EarthLocation(lat=self.latitude,
                                      lon=-self.longitude,
                                      height=self.elevsea)
    ...
    def sun(self, times):
        ...
        altAzFrame = AltAz(obstime=times,
    ...
    def plotSun(self, jd, ax=None, threshold=-12.0, color='black'):
        ...
        for i in range(len(starts)):
            plt.axvspan(starts[i].plot_date, finishes[i].plot_date, color=color, alpha=0.8, zorder=10000)
    ...
    def plot_empty_night(self, midnight, threshold=-12.0, ax=None):
        ...
        assert((finishes > starts).all())
        for i in range(len(starts)):
            plt.axvspan(starts[i].plot_date, finishes[i].plot_date, color='black', alpha=0.8, zorder=10000)
