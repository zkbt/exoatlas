...
class Transit(Talker):
    ...
    def __init__(self, planet,  i, phasefrommidtransit=0.0, observatory=None, buffer=1.5):
        '''
        initialize:
        ...
        self.midtransit = (planet.period*(i + phasefrommidtransit) + planet.transit_epoch)[0]
        self.i = i
    ...
    def plot(self, alpha=0.6, linewidth=5, marker=None, color='black', **kwargs): # y=0,
        ...
        plt.plot(astropy_times.plot_date[ok*intransit], airmass[ok*intransit], color=color, **kwargs)

        # add a label to this transit
        y = np.min(airmass[ok*intransit])-0.05
        description = f'{self.planet.name[0]}\n'
        description += fr'{self.planet.transit_depth[0]:.2%},{self.planet.radius[0].value:.1f}$R_\oplus$,{self.planet.stellar_radius[0].value:.1f}$R_*$' + '\n'
        description += fr'{self.planet.relative_insolation[0]:.0f}$S_\oplus$,{self.planet.distance[0]:.0f}'
        plt.text(Time(self.midtransit, format='jd').plot_date, y,
                 description, color=color,
                 ha='center', va='bottom', weight='bold', alpha=kwargs['alpha'], fontsize=5)
    ...
    def airmass(self, time):
        ...
        return '{0:.1f}'.format(self.observatory.altaz(self.coord, astropy_time).secz.value[0])
