...
`exoatlas` does not intend to replace any of these important archival efforts or their web plotting portals. The main gap that `exoatlas` addresses is to support non-standard calculations and creative visualizations by users uninterested in carefully parsing complicated tables, calculating replacements for missing quantities, or manually propagating uncertainties into derived quantities. By trying to do a good job of these steps behind the scenes, `exoatlas` aims to provide an approachable interface for exploratory analysis, centering a strong focus on scientific storytelling and illuminating visualizations. The target audience is the user who wants to curiously explore one step beyond a web plot, to playfully interact with exoplanet and Solar System data in Python, to fussily adjust how data are filtered or plotted, and to easily integrate new planet discoveries into their workflow.
